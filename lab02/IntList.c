...
void freeIntList(IntList L){
   ...
   if(L->first != NULL){
      ...
      struct IntListNode *previous = L->first;
...
void IntListInsertInOrder(IntList L, int v){
   ...
   //newNode must be inserted at front of list
   }else if( v <= L->first->data){    
      newNode->next = L->first;
      L->first = newNode; 
   
   #ifdef DEBUG
      printf("Inserting node into front of list;\n");
   #endif
   
   //newnode must be inserted into middle or end of list
   }else{
      
      while((current->next != NULL) && (v > current->next->data)){
         current = current->next;   
      }   
   ...
   #ifdef DEBUG
      printf("Inserting node into middle/end of list;\n");
   #endif
