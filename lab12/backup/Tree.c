...
void TREEinsert(Tree tree, Item it){
   ...
   //splay balanced insert
      ...
      //printf("THIS SIS THE RIGHT FILE <\n");
...
link insertSplay (link tree, Item item) {
    ...
    if (less (v, key(tree->item))) {
        ...
        if (less (v, key (tree->left->item))) {                      //if key is less than left sub-tree val
    ...
    } else {                                                               // check if empty on the right
        if (tree->right == emptyTree) {                                    // check if right sub tree exists 
